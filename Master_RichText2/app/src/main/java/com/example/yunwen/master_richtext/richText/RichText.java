...
import com.example.yunwen.master_richtext.richText.listener.OnTextViewClickListener;
    ...
    public RichText text(String text) {
        ...
        if (null != urls && urls.length > 0) {
            for (URLSpan url : urls) {
                ...
                CharSequence charSequence = spannableStringBuilder.subSequence(start, end);
                LinkClickSpan myURLSpan = new LinkClickSpan(url.getURL(),charSequence);

                if (null == myURLSpan.getListener()) {
                    myURLSpan.setListener(onTextViewClickListener);
                }

                Log.e("AA", "当前的文字内容为： " + charSequence);
...
//                        start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    class LinkClickSpan extends ClickSpan {
        ...
        public LinkClickSpan(String url, CharSequence charSequence) {
            super(url);
        ...
        public void onClick(View widget) {
            ...
            if (null != this.listener) {
                this.listener.textLinkClicked(url, charSequence.toString());
            }
        }
    }
