        ...
        Glide.with(mConText).load(source).asBitmap().into(new SimpleTarget<Bitmap>() {
            ...
            public void onLoadFailed(Exception e, Drawable errorDrawable) {
                ...
                if (null != adapter && (errorImage = adapter.getErrorDrawable()) != null) {
                    urlDrawable.setDrawable(errorImage, container.getMeasuredHeight() - container.getPaddingTop() - container.getPaddingBottom());
                }
            }
        });
...
//            int height_px = MoonDensityUtil.dip2px(container.getContext(), height_dp);
            int width_px = MoonDensityUtil.dip2px(container.getContext(), 120);
            int height_px = MoonDensityUtil.dip2px(container.getContext(), 120);


//            //too large,adapter screen,but the maxwidth can be zero.
//            if (maxWidth > 0) {
//                height_px = height_px > maxWidth ? height_px * (width_px / maxWidth) : height_px;
//                width_px = width_px > maxWidth ? maxWidth : width_px;
//            }
            if (maxWidth > 0) {
                height_px = height_px > maxWidth ? maxWidth : height_px;
                width_px = width_px > maxWidth ?  height_px * (width_px / maxWidth) : width_px;
            }
